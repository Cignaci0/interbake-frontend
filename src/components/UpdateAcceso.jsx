...
import updateDispositivoService from "../services/updateDispositivoService";

export default function UpdateAcceso({ acceso, onSuccess, onCancel }) {
  ...
  useEffect(() => {
    if (acceso) {
      setFormData({
        ...
        comuna: acceso.comuna || "",
        modelo: acceso.modelo || "",
        fabricante: acceso.fabricante || "",
        version_firmware: acceso.version_firmware || "",
        direccion_ip: acceso.direccion_ip || "",
        gateway: acceso.gateway || "",
        dns: acceso.dns || "",
        estado: String(acceso.estado ?? "1")
      });
    }
  }, [acceso]);
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    const toastId = toast.loading("Actualizando acceso...");
    ...
    try {
      ...
      await updateDispositivoService(acceso.dispositivo_id, payload);

      toast.success("Acceso actualizado exitosamente", { id: toastId });
      onSuccess();
    } catch (error) {
      toast.error(error.response?.data?.message || "Error al actualizar el acceso", { id: toastId });
      console.error(error);
    } finally {
      setLoading(false);
    }
